classdef SVMModel < handle & modelToolkit
    ...
    methods
        function obj = SVMModel(kfold, penaltyList,  thresholdList, discription)
            obj.KFOLD = kfold;
...
%           libsvmOption = strcat( '-q -s 0 -t 0 -b 1 -w0 1 -w1',32 ,num2str(parameter.penalty));
            model =  svmtrain(trainData(:,1), trainData(:, FeatureSet), libsvmOption);
...
%             Featureset = [39:42]
            ...
            [result, ~, probResult] = svmpredict(testData(:, 1), testData(:, Featureset), Model, '-b 1' );
        ...
        function [result, probResult]=testRecordedModelwithThreshold(obj, testData, curFold)
            ...
            for piter = 1:length(obj.thresholdList)
                ...
                for titer = 1:size(probResult)
                    ...
                    if probResult(titer, 1)> threshold
                        resultT(titer, 1) = 1;
                    else
                        resultT(titer, 1) = 0;
                    end
